...
import { Users, Search, Filter, Plus, User, Star, Award, Clock, TrendingUp, Sparkles, X, Phone, Mail, Calendar, Briefcase } from "lucide-react";

export function PersonnelPage() {
  const [view, setView] = useState<"grid" | "matrix" | "shifts">("grid");
  ...
  return (
    <div className="p-3 md:p-5 lg:p-6 space-y-3 md:space-y-4 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
        {[
          { label: "کل پرسنل", value: stats.total, color: "#8b5cf6", icon: Users },
          { label: "فعال", value: stats.active, color: "#22c55e", icon: User },
          { label: "میانگین بهره‌وری", value: stats.avgProductivity + "%", color: "#d4a017", icon: TrendingUp },
          { label: "کل دستور کارها", value: stats.totalWorkOrders, color: "#3b82f6", icon: Briefcase },
        ].map((s, i) => {
          const Icon = s.icon;
          return (
      ...
      <div className="chart-card !p-3 flex flex-col md:flex-row gap-2">
        ...
        <div className="flex gap-2">
          <select value={filterRole} onChange={e => setFilterRole(e.target.value)} className="select-field flex-1 md:w-[140px] md:flex-initial">
          ...
          <button className="btn-primary flex-1 md:flex-initial justify-center">
            <Plus className="w-4 h-4" /> افزودن پرسنل
          </button>
        </div>
      </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-0.5 mt-1">
                    ...
                    <span className="text-[9px] text-gray-500 mr-1">({p.rating})</span>
                  </div>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-2 mt-3 pt-3 border-t border-gray-200 dark:border-[#1a1a1a] text-center">
                <div>
                  <p className="text-[9px] text-gray-500">بهره‌وری</p>
                  <p className="text-sm font-bold text-green-500">{p.productivity}%</p>
                </div>
                <div>
                  <p className="text-[9px] text-gray-500">دستور کار</p>
                  <p className="text-sm font-bold text-amber-500">{p.completedWorkOrders}</p>
                </div>
                <div>
                  <p className="text-[9px] text-gray-500">مهارت</p>
                  <p className="text-sm font-bold text-blue-500">{p.skills.length}</p>
                </div>
              </div>
            </button>
          ))}
      ...
      {view === "matrix" && (
        <div className="chart-card">
                  ...
                  <tr key={p.id} className="border-b border-gray-100 dark:border-[#0a0a0a] table-row-hover">
                    <td className="py-2 px-2 sticky right-0 bg-white dark:bg-[#111]">
                      <div className="flex items-center gap-2">
                        <div className="w-7 h-7 rounded-lg bg-amber-500/20 flex items-center justify-center text-[10px] font-bold text-amber-500">
                          {p.fullName.charAt(0)}
                        </div>
                        <div>
                              ...
                              <div className="flex">
                                {[1,2,3,4,5].map(i => (
                                  <div
                                    ...
                                    style={{ backgroundColor: i <= s.level ? skillCategoryColors[skill.category] : '#e5e7eb' }}
                                  />
                                ))}
                              </div>
          ...
          <div className="flex flex-wrap gap-2 mt-3 text-[10px]">
            ...
            <span>۱: مبتدی • ۲: پایه • ۳: متوسط • ۴: پیشرفته • ۵: خبره</span>
          </div>
        </div>
      )}
                ...
                <div className="space-y-1.5">
                  {shiftPersonnel.map(p => (
                    <button key={p.id} onClick={() => setSelected(p)} className="w-full flex items-center gap-2 p-2 rounded-lg bg-gray-50 dark:bg-[#0a0a0a] hover:bg-amber-500/10 text-right">
                      <div className="w-8 h-8 rounded-full bg-amber-500 flex items-center justify-center text-[#0a0a0a] font-bold text-xs">
                        {p.fullName.charAt(0)}
                      </div>
                      <div className="flex-1 min-w-0">
        ...
        <div className="p-4 space-y-4">
          <div className="grid grid-cols-3 gap-2 text-xs">
            <div className="p-2 bg-gray-50 dark:bg-[#0a0a0a] rounded-lg text-center">
              <p className="text-[10px] text-gray-500">بهره‌وری</p>
              <p className="font-black text-green-500 text-lg">{person.productivity}%</p>
            </div>
            <div className="p-2 bg-gray-50 dark:bg-[#0a0a0a] rounded-lg text-center">
              <p className="text-[10px] text-gray-500">دستور کار</p>
              <p className="font-black text-amber-500 text-lg">{person.completedWorkOrders}</p>
            </div>
            <div className="p-2 bg-gray-50 dark:bg-[#0a0a0a] rounded-lg text-center">
              <p className="text-[10px] text-gray-500">امتیاز</p>
              <p className="font-black text-blue-500 text-lg">{person.rating}</p>
            </div>
          </div>
          ...
          <div className="space-y-2 text-xs">
            ...
            <div className="flex items-center gap-2"><Briefcase className="w-3.5 h-3.5 text-gray-500" /><span>دپارتمان: {person.department}</span></div>
          </div>
        </div>
      </div>
    </div>
